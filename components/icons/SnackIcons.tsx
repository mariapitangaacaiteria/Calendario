...
export const SnackProvider: React.FC<SnackProviderProps> = ({ children }) => {
  ...
  const [mounted, setMounted] = useState(false); // <<< evita hydration warning do portal
  ...
  const createSnack = useCallback((message: React.ReactNode, variant: Variant) => {
    ...
    timeoutsRef.current.push(hideId, removeId);
  }, []);
  ...
  return (
    <SnackContext.Provider value={{ createSnack }}>
      {children}
      {/* Renderiza o portal somente após montar no client */}
...
const getVariantIcon = (variant: Variant) => {
  ...
  switch (variant) {
    case 'success':
      return (
        <svg className={`${base} text-green-500`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
          <path fillRule="evenodd" d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10S2 17.523 2 12Zm13.707-1.293a1 1 0 0 0-1.414-1.414L11 12.586l-1.793-1.793a1 1 0 0 0-1.414 1.414l2.5 2.5a1 1 0 0 0 1.414 0l4-4Z" clipRule="evenodd"/>
        </svg>
      );
    case 'error':
      return (
        <svg className={`${base} text-red-500`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
